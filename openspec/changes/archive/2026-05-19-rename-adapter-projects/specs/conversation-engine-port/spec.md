...
- **THEN** its project references SHALL NOT include `Anamnesis.Adapter.Llm.Ollama` or `Anamnesis.Adapter.Llm.Contract`
...
All prompt definitions, conversation history management (including system prompt insertion), and LLM response parsing SHALL reside exclusively in the Ollama adapter project (`Anamnesis.Adapter.Llm.Ollama`). No prompt string, LLM message role value, or LLM response parsing logic SHALL exist in the use case or domain layers.
